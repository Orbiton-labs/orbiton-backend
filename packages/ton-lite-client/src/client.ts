...
import {
  ...
  AllShardsResponse,
  BlockID,
  BlockLookupIDRequest,
  BlockLookupRequest,
  BlockLookupUtimeRequest,
  CacheMap,
  ClientAccountState,
  QueryArgs,
} from './types';
  ...
  constructor(opts: {
    ...
    this.#accounts = new DataLoader<AccountsDataLoaderKey, ClientAccountState, string>(
      async (s) => {
        return await Promise.all(
          s.map((v) =>
            this.getAccountStateRaw(v.address, {
              fileHash: v.fileHash,
              rootHash: v.rootHash,
              seqno: v.seqno,
              shard: v.shard,
      ...
      if (accountSlice.loadBit()) {
        ...
        if (account) {
          ...
          if (shardState.accounts) {
            ...
            if (pstate) {
              lastTx = {
                hash: pstate.shardAccount.lastTransactionHash,
                lt: pstate.shardAccount.lastTransactionLt,
              };
            }
          }
        }
