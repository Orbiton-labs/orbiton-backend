...
export interface DispatchQueue {
  ...
  readonly anon0: Dictionary<bigint, { value: AccountDispatchQueue; extra: number }>;
}
...
export function loadMessage<X>(slice: Slice, loadX: (slice: Slice) => X): Message<X> {
  ...
  let init: Maybe<Either<StateInit, StateInit>> = loadMaybe<Either<StateInit, StateInit>>(
    slice,
    (slice: Slice) => {
...
export function loadInMsgDescr(slice: Slice): InMsgDescr {
  let anon0: Dictionary<bigint, { value: InMsg; extra: ImportFees }> = Dictionary.load(
    Dictionary.Keys.BigUint(256),
    {
      serialize: () => {
        throw new Error('Not implemented');
...
export function loadOutMsgDescr(slice: Slice): OutMsgDescr {
  let anon0: Dictionary<bigint, { value: OutMsg; extra: CurrencyCollection }> = Dictionary.load(
    Dictionary.Keys.BigUint(256),
    {
      serialize: () => {
        throw new Error('Not implemented');
...
export function loadOutMsgQueue(slice: Slice): OutMsgQueue {
  let anon0: Dictionary<bigint, { value: EnqueuedMsg; extra: number }> = Dictionary.load(
...
export function loadDispatchQueue(slice: Slice): DispatchQueue {
  let anon0: Dictionary<bigint, { value: AccountDispatchQueue; extra: number }> = Dictionary.load(
    Dictionary.Keys.BigUint(256),
    {
      serialize: () => {
        throw new Error('Not implemented');
...
export function storeDispatchQueue(dispatchQueue: DispatchQueue): (builder: Builder) => void {
  return (builder: Builder) => {
    builder.storeDict(dispatchQueue.anon0, Dictionary.Keys.BigUint(256), {
      serialize: (arg: { value: AccountDispatchQueue; extra: number }, builder: Builder) => {
        ((arg: number) => {
          return (builder: Builder) => {
            builder.storeUint(arg, 64);
          };
...
export function loadOutMsgQueueExtra(slice: Slice): OutMsgQueueExtra {
  if (slice.remainingBits >= 4 && slice.preloadUint(4) == 0x0) {
    ...
    let out_queue_size: Maybe<number> = loadMaybe<number>(slice, (slice: Slice) => {
...
export function loadShardAccounts(slice: Slice): ShardAccounts {
  let anon0: Dictionary<bigint, { value: ShardAccount; extra: DepthBalanceInfo }> = Dictionary.load(
    Dictionary.Keys.BigUint(256),
    {
      serialize: () => {
        throw new Error('Not implemented');
...
export function loadAccountBlock(slice: Slice): AccountBlock {
  if (slice.remainingBits >= 4 && slice.preloadUint(4) == 0x5) {
    slice.loadUint(4);
    let account_addr: BitString = slice.loadBits(256);
    let transactions: HashmapAug<Transaction, CurrencyCollection> = loadHashmapAug<
      Transaction,
      CurrencyCollection
    >(
      slice,
...
export function loadShardAccountBlocks(slice: Slice): ShardAccountBlocks {
  let anon0: Dictionary<bigint, { value: AccountBlock; extra: CurrencyCollection }> =
    Dictionary.load(
      Dictionary.Keys.BigUint(256),
      {
        serialize: () => {
...
export function loadShardFees(slice: Slice): ShardFees {
  let anon0: Dictionary<bigint, { value: ShardFeeCreated; extra: ShardFeeCreated }> =
    Dictionary.load(
      Dictionary.Keys.BigUint(96),
      {
        serialize: () => {
...
export function loadOldMcBlocksInfo(slice: Slice): OldMcBlocksInfo {
  let anon0: Dictionary<number, { value: KeyExtBlkRef; extra: KeyMaxLt }> = Dictionary.load(
    Dictionary.Keys.Uint(32),
    {
      serialize: () => {
        throw new Error('Not implemented');
...
export function loadBlockCreateStats(slice: Slice): BlockCreateStats {
  ...
  if (slice.remainingBits >= 8 && slice.preloadUint(8) == 0x34) {
    slice.loadUint(8);
    let counters: Dictionary<bigint, { value: CreatorStats; extra: number }> = Dictionary.load(
      Dictionary.Keys.BigUint(256),
      {
        serialize: () => {
          throw new Error('Not implemented');
...
export function loadMcBlockExtra(slice: Slice): McBlockExtra {
  if (slice.remainingBits >= 16 && slice.preloadUint(16) == 0xcca5) {
    ...
    let slice1 = slice.loadRef().beginParse(true);
    let prev_blk_signatures: Dictionary<number, CryptoSignaturePair> = Dictionary.load(
