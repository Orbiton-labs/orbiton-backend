...
import { loadBit, loadUint, loadUintLeq, loadMaybeRef } from '../blockchain/BlockUtils.js';
...
export class Hashmap {
  ...
  serializeBitmap(cell, map, n) {
    ...
    if (label.length === 0 || label === 'one') {
      ...
      if (
        label.split('0').length - 1 === label.length ||
        label.split('1').length - 1 === label.length
      ) {
        // hml_same$11 {m:#} v:Bit n:(#<= m) = HmLabel ~n m;
