...
export class Cell {
  ...
  async finalize() {
      ...
      case Cell.PrunnedBranchCell:
        {
          ...
          if (this.bits.length < (2 + hashes * (hash_bytes + depth_bytes)) * 8) {
            console.log({
              ...
              '(2 + hashes * (hash_bytes + depth_bytes)) * 8':
                (2 + hashes * (hash_bytes + depth_bytes)) * 8,
              hashes,
              hash_bytes,
              depth_bytes,
          ...
          if (!compareBytes(merkleHash, childHash)) {
            throw Error(
              'Hash mismatch in a MerkleProof special cell ' +
                bytesToHex(merkleHash) +
                ' != ' +
                bytesToHex(childHash),
            );
          }
      ...
      case Cell.MerkleUpdateCell:
        {
          ...
          if (!compareBytes(merkleHash0, childHash0)) {
            throw Error(
              'First hash mismatch in a MerkleUpdate special cell ' +
                bytesToHex(merkleHash0) +
                ' != ' +
                bytesToHex(childHash0),
            );
          }
          ...
          if (!compareBytes(merkleHash1, childHash1)) {
            throw Error(
              'Second hash mismatch in a MerkleUpdate special cell ' +
                bytesToHex(merkleHash1) +
                ' != ' +
                bytesToHex(childHash1),
            );
          }
          ...
          if (
            this.bits.readUint16(8 + 16 * hash_bytes + depth_bytes * 8) != this.refs[1].getDepth(0)
          ) {
    ...
    for (let level_i = 0, hash_i = 0, level = this.getLevel(); level_i <= level; level_i++) {
      ...
      if (hash_i == hash_i_offset) {
        ...
        if (level_i != 0 && type != Cell.PrunnedBranchCell) throw Error('Cannot deserialize cell');
  ...
  async toBoc(has_idx = true, hash_crc32 = true, has_cache_bits = false, flags = 0) {
    ...
    topologicalOrder.sort((a, b) => {
      return cellsIndex[a[0]][0] - cellsIndex[b[0]][0];
    });
    // console.log('sorted');
    ...
    const serialization = new BitString(
      (1023 * s_bytes + 32 * 4 + 32 * 3) * topologicalOrder.length,
    );
...
function parseBocHeader(serializedBoc) {
  ...
  if (serializedBoc.length < 4 + 1) throw Error('Not enough bytes for magic prefix');
  ...
  if (hash_crc32) {
    if (serializedBoc.length < 4) throw Error('Not enough bytes for crc32c hashsum');
  ...
  if (serializedBoc.length) throw Error('Too much bytes in BoC serialization');
  return {
    has_idx: has_idx,
...
function deserializeCellData(cellData, referenceIndexSize) {
  if (cellData.length < 2) throw Error('Not enough bytes to encode cell descriptors');
