...
export class Block {
  ...
  async getLatestId() {
    ...
    try {
      ...
      if (!res) throw Error('empty answer');
  ...
  async lookup(blockId, lt, utime) {
    let result = { ok: false };
    try {
      if (!blockId) throw Error('Block has no id');
      ...
      if (blockProofCell.length !== 1) throw Error('Invalid root');

      // check root_hash
      if (
        blockProofCell[0].type !== Cell.MerkleProofCell ||
      ...
      // check other
      if (!(lt || utime) && blockHeader.info.seq_no !== blockId.seqno) throw Error('Invalid seqno');
      if (!blockId.compareShard(blockHeader.info.shard.shard)) throw Error('Invalid shard');
      ...
      if (utime && blockHeader.info.gen_utime !== utime) throw Error('Invalid utime');
  ...
  async getHeader(blockId) {
    ...
    try {
      ...
      if (
        blockProofCell[0].type !== Cell.MerkleProofCell ||
        !compareBytes(
          new Uint8Array(blockId.root_hash.buffer),
          blockProofCell[0].refs[0].getHash(0),
        )
      )
        throw Error('Invalid root_hash');
      ...
      // check other
      if (blockHeader.info.seq_no !== blockId.seqno) throw Error('Invalid seqno');
      if (!blockId.compareShard(blockHeader.info.shard.shard)) throw Error('Invalid shard');
      if (blockHeader.info.shard.workchain_id !== blockId.workchain)
        throw Error('Invalid workchain');
      if (blockHeader.info.not_master != (blockId.workchain !== -1))
  ...
  async getData(blockId) {
    ...
    try {
      ...
      if (!res) throw Error('Cannot get block data');
      ...
      if (blockCell.length !== 1) throw Error('Invalid root');
      ...
      // check other
      if (block.info.seq_no !== blockId.seqno) throw Error('Invalid seqno');
      if (!blockId.compareShard(block.info.shard.shard)) throw Error('Invalid shard');
      if (block.info.shard.workchain_id !== blockId.workchain) throw Error('Invalid workchain');
      if (block.info.not_master != (blockId.workchain !== -1)) throw Error('Invalid not_master');
  ...
  async _compute_validators_set(gen, validators, count, shuffle_mc_val) {
    ...
    if (shuffle_mc_val) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        nodes.push({
          key: v.public_key.pubkey,
          weight: v.weight,
          addr: v.adnl_addr ? v.adnl_addr : new Uint8Array(32),
        });
      }
    } else {
  ...
  async validate(blockId) {
    ...
    try {
      ...
      if (!from) throw Error('No known blocks');
      ...
      for (let i = 0; i < 10000; i++) {
        ...
        if (!from.compare(blockProof.from)) throw Error('Invalid response');

        if (
          blockProof.from.workchain !== -1 ||
          !blockId.compareShard(blockProof.from.shard) ||
          blockProof.to.workchain !== -1 ||
          !blockId.compareShard(blockProof.to.shard)
        )
          throw Error('BlockProof must have both source and destination blocks in the masterchain');

        if (blockProof.steps.length < 1) throw Error('BlockProof length 0');
        ...
        for (let k of blockProof.steps) {
          ...
          if (!curr.compare(k.from)) throw Error('Invalid BlockProof chain');

          if (
            k.from.workchain !== -1 ||
            !curr.compareShard(k.from.shard) ||
            k.to.workchain !== -1 ||
            !curr.compareShard(k.to.shard)
          )
            throw Error(
          ...
          if (fwd !== k.to.seqno > k.from.seqno) throw Error('BlockProofLink invalid type');
          ...
          if (proof.length === 0) throw Error('BlockProofLink contains no proof for source block');
          ...
          if (k.from.seqno > 0) {
            ...
            if (
              proofCell[0].type != 3 ||
              !compareBytes(
                proofCell[0].refs[0].getHash(0),
                new Uint8Array(k.from.root_hash.buffer),
              )
            )
          ...
          if (!fwd) {
            ...
            if (
              stateProofCell.type != 3 ||
              !compareBytes(state_hash, stateProofCell.refs[0].getHash(0))
            )
            ...
            if (prevBlock.value.blk_ref.seq_no !== k.to.seqno) throw Error('Invalid dest seqno');

            if (
              !compareBytes(
                prevBlock.value.blk_ref.file_hash,
                new Uint8Array(k.to.file_hash.buffer),
              )
            )
            ...
            if (
              !compareBytes(
                prevBlock.value.blk_ref.root_hash,
                new Uint8Array(k.to.root_hash.buffer),
              )
            )
              throw Error('Invalid dest file_hash');
          } else {
            ...
            const nodes = await this._compute_validators_set(
              prng,
              validators.list.map,
              count,
              catchainConfig.shuffle_mc_validators,
            );
            ...
            if (signed_weight.lte(total_weight)) throw Error('insufficient total signature weight');
          }

          curr = k.to;

          if (k.to_key_block) this.storage.addBlock(k.to);
        }
  ...
  async getShards(blockId) {
    ...
    try {
      if (!blockId) throw Error('Block has no id');
      if (blockId.workchain !== -1) throw Error('Block is not a masterchain');
      ...
      if (blockProofCell.length !== 2) throw Error('Invalid root');
      ...
      if (
        blockProofCell[1].type !== Cell.MerkleProofCell ||
  ...
  async getTransactions(maxCount, accountAddr, lt, hash, to_lt) {
    let result = { ok: false };
    try {
      if (to_lt !== undefined && lt.lte(to_lt)) throw Error('lt must be > than to_lt');
      ...
      for (let r = 0; r < requests; r++) {
        ...
        if (transactionsCell.length !== res.ids.length) throw Error('Invalid answer');
        ...
        if (to_lt_reached || last_lt.lte(new BN(0))) break;
  ...
  static _shardGetFromHashmap(m, wc) {
    if (!m.map.has(wc.toString())) return;

    let bt = m.map.get(wc.toString());
    return this.getLeafFromBinTree(bt).flat(1);
  }

  static getLeafFromBinTree(bt) {
    if (!bt.type) return;
    if (bt.type === 'leaf') return bt.leaf;
    if (bt.type === 'fork') {
      const btLeftData = this.getLeafFromBinTree(bt.left);
      const btRightData = this.getLeafFromBinTree(bt.right);
      let leaves = [];
      if (btLeftData) leaves.push(btLeftData);
      if (btRightData) leaves.push(btRightData);
      return leaves;
    }
  }
  ...
  async getAccountState(blockId, accountAddr) {
    ...
    try {
      ...
      if (!state) throw Error('Cannot get account state');

      let shardBlockId;
      if (address.wc !== -1) {
        if (state.shard_proof.length === 0) throw Error('No shard proof');

        const shardProofCell = await Cell.fromBoc(state.shard_proof);

        if (shardProofCell.length !== 2) throw Error('Invalid block root');

        // check root_hash
        if (
          shardProofCell[0].type !== Cell.MerkleProofCell ||
          !compareBytes(
            new Uint8Array(blockId.root_hash.buffer),
            shardProofCell[0].refs[0].getHash(0),
          )
        )
        ...
        // check other
        if (shardBlockHeader.info.seq_no !== blockId.seqno) throw Error('Invalid seqno');
        if (!blockId.compareShard(shardBlockHeader.info.shard.shard)) throw Error('Invalid shard');
        ...
        // check MC ShardState hash
        if (
          shardProofCell[1].type !== Cell.MerkleProofCell ||
        ...
        const shardDescr = Block._shardGetFromHashmap(
          mcShardState.custom.shard_hashes,
          address.wc,
          address.hashPart,
        );
        if (!shardDescr) throw Error('No account shard found');
      ...
      if (blockProofCell.length !== 2) throw Error('Invalid shard root');

      // check root_hash
      if (
        blockProofCell[0].type !== Cell.MerkleProofCell ||
        !compareBytes(
          new Uint8Array(shardBlockId.root_hash.buffer),
          blockProofCell[0].refs[0].getHash(0),
        )
      )
      ...
      // check other
      if (blockHeader.info.seq_no !== shardBlockId.seqno) throw Error('Invalid seqno');
      ...
      const accountHash =
        accountValue.account._ === 'Cell'
          ? accountValue.account.getHash(0)
          : accountValue.account.cell.getHash(0);

      // get & check account hash
      if (
        accountCell.type !== Cell.OrdinaryCell ||
  ...
  async getConfig(blockId, configNum) {
    ...
    try {
      ...
      let res;
      if (configNum === undefined) res = await this.provider.getConfigAll(blockId);
      else res = await this.provider.getConfigParams(blockId, [configNum]);
      if (!res) throw Error('empty answer');
      ...
      // check MC ShardState hash
      if (
        configProofCell[0].type !== Cell.MerkleProofCell ||
      ...
      if (!mcState.custom.config.config.map.has(configNum.toString(16))) {
        ...
        while (configBin.length < 32) {
          configBin = '0' + configBin;
        }
  ...
  compute_method_id(method) {
    let method_id;
    try {
      if (method.startsWith('0x')) method_id = new BN(method.replace('0x', ''), 16);
      else method_id = new BN(method, 10);
    } catch (e) {
  ...
  async runSmcMethod(blockId, accountAddr, method, params) {
    ...
    try {
      ...
      if (!res) throw Error('Cannot get account state');
  ...
  async sendMessage(message) {
    let result = { ok: false };
    try {
      if (typeof message == 'string') message = base64ToBytes(message);

      let res = await this.provider.sendMessage(message);
      if (!res) throw Error('Cannot send message');
