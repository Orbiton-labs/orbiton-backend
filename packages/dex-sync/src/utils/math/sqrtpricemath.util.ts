...
import { FullMath } from "./fullmath.util"; // Assuming FullMath is defined elsewhere and imported correctly
import { MAX_TICK, MaxUint256, MIN_TICK } from "../dex.util";
...
function toBigInt(val: JSBI): bigint {
...
export class SqrtPriceMath {
  public static getAmount0Delta(
    ...
    let sqrtRatioAX96JSBI = toJSBI(sqrtRatioAX96);
    let sqrtRatioBX96JSBI = toJSBI(sqrtRatioBX96);
    const liquidityJSBI = toJSBI(liquidity);

    if (JSBI.greaterThan(sqrtRatioAX96JSBI, sqrtRatioBX96JSBI)) {
      [sqrtRatioAX96JSBI, sqrtRatioBX96JSBI] = [
        sqrtRatioBX96JSBI,
        sqrtRatioAX96JSBI,
      ];
    }

    const numerator1 = JSBI.leftShift(liquidityJSBI, JSBI.BigInt(96));
    const numerator2 = JSBI.subtract(sqrtRatioBX96JSBI, sqrtRatioAX96JSBI);
    ...
    return roundUp
      ...
      : toBigInt(
          JSBI.divide(
            JSBI.divide(
              JSBI.multiply(numerator1, numerator2),
              sqrtRatioBX96JSBI
            ),
            sqrtRatioAX96JSBI
          )
  ...
  public static getAmount1Delta(
    ...
    const delta = JSBI.subtract(sqrtRatioBX96JSBI, sqrtRatioAX96JSBI);

    return roundUp
      ? toBigInt(FullMath.mulDivRoundingUp(liquidityJSBI, delta, Q96))
      : toBigInt(JSBI.divide(JSBI.multiply(liquidityJSBI, delta), Q96));
  }

  static mulShift(val: JSBI, mulBy: string): JSBI {
  ...
  static getSqrtRatioAtTick(tick: number): bigint {
    ...
    if (tick > 0)
      ratio = JSBI.divide(JSBI.BigInt(MaxUint256.toString()), ratio);

    return BigInt(
      (JSBI.greaterThan(JSBI.remainder(ratio, Q32), JSBI.BigInt(0))
